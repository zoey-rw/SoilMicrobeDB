...
# Use species-level classification data from 06_calculate_classification_pct_by_rank.r
# This replaces the old pass_filter_summary.csv with updated species-level percentages
...
cat("Loading species-level classification data from:", classification_file, "\n")
classification_data <- read_csv(classification_file, show_col_types = FALSE)

# Filter to species-level, all_domain, after filtering
# Convert to match old format structure
pass_filter_species_long <- classification_data %>%
    ...
    mutate(
        ...
        value = pct_classified / 100,  # Convert from percentage to proportion
        pretty_metric = "% reads classified at\n high quality",
        seq_depth = seq_depth
    ) %>%
    select(sampleID, db_name, metric, value, pretty_metric, seq_depth)
...
fig2 = pass_filter_species_long  %>% 
    ...
    stat_compare_means(aes(group = db_name), show.legend = F)  + 
...
# Note: soilCores comes from NEON data RDS file
